...
var AlanLogin =  Dialog.extend({
    ...
    _userSignup:function(ev){
        ...
        const first_name = cr.$el.find("#names").val();
        const last_name = cr.$el.find('#last_names').val();
        const confirm_passwords = cr.$el.find("#confirm_passwords").val();
        const token = cr.$el.find("#token").val()
        const gender = cr.$el.find("#gender").val();
        const dobs = cr.$el.find("#dob").val();
        const names = first_name.concat(" ",last_name);
...
publicWidget.registry.as_core = publicWidget.Widget.extend(AlanTemplateGetter,
    ...
    _quickVariantView:function(ev, hasVariant, productId){
        ...
        return this._rpc({
            ...
            qac.opened(() =>{
                self.trigger_up('widgets_start_request', {
                    $target: qac.$modal.find(".show-extra-prod-info"),
                });
            })
            $(ev.currentTarget).removeClass("as-btn-loading");
        });
...
publicWidget.registry.as_product_details = publicWidget.Widget.extend(AjaxAddToCart, {
    ...
    'events':{
        ...
        'scroll':'_stickyCart',
        'click a#add_to_cart':'_prodAddToCart',
    },
    start: function () {
        return this._super.apply(this, arguments).then(() => {
    ...
    _stickyCart:function(ev){
     ...
     _prodAddToCart: function(ev){
        var product_id = $(ev.currentTarget).closest("form").find("input[name='product_id']").val();
        cloneAnimation($('header .o_wsale_my_cart').first(), this.$(ev.currentTarget).parent().parent().parent().parent().parent().find('.active').find('.img-gallery-tag'), 25, 40);
    },
});
